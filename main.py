...
def main(page: ft.Page):
    # THE MOST STABLE SETUP FOR ANDROID
    page.title = "GGGM Stable"
    page.theme_mode = ft.ThemeMode.LIGHT
    page.padding = 0
    page.bgcolor = "#3F51B5" # Indigo

    # Application State
    state = {"view": "lyrics", "q": ""}
    
    # Top-level container that we will "Swap" content in
    ...
    def get_nav():
        return ft.NavigationBar(
            ...
            selected_index=0 if state["view"]=="lyrics" else 1 if state["view"]=="bible" else 2,
            on_change=lambda e: (state.update({"view": ["lyrics", "bible", "settings"][e.control.selected_index]}), render())
        )

    def render():
        # Clear the inner container, NOT the whole page
        ...
        if state["view"] == "lyrics":
            ...
            main_container.content = ft.Column([
                ft.Container(content=ft.Column([
                    ft.Text("Grace Lyrics", size=26, color="white", weight="bold"),
                ...
                ft.ListView([ft.ListTile(title=ft.Text(s["title"]), leading=ft.Icon(ft.Icons.MUSIC_VIDEO), on_click=lambda e, s=s: show_details(s)) for s in songs], expand=True)
            ], expand=True)

        elif state["view"] == "bible":
            books = bible_eng.get_list()
            grid = ft.GridView(expand=True, runs_count=2, spacing=10, padding=10)
            for b in books:
                grid.controls.append(ft.Container(content=ft.Text(b["name"], weight="bold", text_align="center"), bgcolor="white", border_radius=12, alignment=ft.alignment.center, height=80, on_click=lambda e, bk=b: show_read(bk)))
            main_container.content = ft.Column([
                ft.Container(content=ft.Text("Sacred Bible", size=26, color="white", weight="bold"), padding=20, bgcolor="#3F51B5"),
                grid
            ], expand=True)

        elif state["view"] == "settings":
            main_container.content = ft.Container(content=ft.Text("Stable Mode v1.2.0"), padding=20)
    ...
    def show_details(s):
        page.clean()
        page.add(ft.Column([
            ft.AppBar(title=ft.Text(s["title"]), bgcolor="#3F51B5", color="white", leading=ft.IconButton(ft.Icons.ARROW_BACK, icon_color="white", on_click=lambda _: back_to_home())),
    ...
    def show_read(book):
        ...
        page.clean()
        page.add(ft.Column([
            ft.AppBar(title=ft.Text(book["name"]), bgcolor="#3F51B5", color="white", leading=ft.IconButton(ft.Icons.ARROW_BACK, icon_color="white", on_click=lambda _: back_to_home())),
            ft.Container(content=ft.Text(verses, size=22), padding=30, expand=True, bgcolor="#FFF9E1", scroll="auto")
        ], expand=True))

    def back_to_home():
        page.clean()
        page.navigation_bar = get_nav()
