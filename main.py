...
def main(page: ft.Page):
    ...
    def show_reader(s):
        page.appbar = ft.AppBar(
            ...
            actions=[
                ft.IconButton(ft.Icons.REMOVE_CIRCLE_OUTLINE, icon_color="white", on_click=lambda _: (st.update({"font_size": max(12, st["font_size"]-2)}), refresh())),
                ft.IconButton(ft.Icons.ADD_CIRCLE_OUTLINE, icon_color="white", on_click=lambda _: (st.update({"font_size": min(40, st["font_size"]+2)}), refresh()))
            ]
        )
    ...
    def show_settings():
        ...
        page.add(ft.Container(padding=30, content=ft.Column([
            ft.Row([ft.Image(src="icon.png", width=100, height=100)], alignment=ft.MainAxisAlignment.CENTER),
            ft.Container(height=20),
            ft.Text("Cloud Sync", size=24, weight="bold", color="black"),
            ft.Text(f"Target Project: {PROJECT_ID}", color="grey"),
            ft.Container(height=20),
            ft.ElevatedButton("SYNC NOW", icon=ft.Icons.SYNC, bgcolor="#1A237E", color="white", on_click=sync_act, height=50),
            ft.Divider(),
            ft.Text("Gods Grace Gospel Ministry v5.3.0", color="grey", size=11, text_align=ft.TextAlign.CENTER)
        ], horizontal_alignment=ft.CrossAxisAlignment.CENTER)))
        page.update()

    def render_home():
        # GHOST BAR FIX: Explicitly disable and clear appbar before rendering home
        if page.appbar:
        ...
        header = ft.Container(
            ...
            content=ft.Column([
                ft.Row([
                    ft.Row([ft.Image(src="icon.png", width=50, height=50), ft.Column([ft.Text("Gods Grace", color="white", size=20, weight="bold", tight=True), ft.Text("Gospel Ministry", color="white", size=12)], spacing=0)]),
